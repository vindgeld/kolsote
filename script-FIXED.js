 ...
 * 🔧 COMPLETE FIXED VERSION - All networking bugs resolved
 * @budibaik (Fixed & Enhanced with PeerJS improvements)
 * ============================================================
 * FIXES INCLUDED:
 * ✅ FIX #1: Connection Heartbeat (PING/PONG) - Detects dead connections
 * ✅ FIX #2: Improved Broadcast Function - Pre-filters before sending
 * ✅ FIX #3: Reconnection Race Condition - Prevents connection storms
 * ✅ FIX #4: Username Persistence on Reconnect - Maintains identity
 * ✅ FIX #5: Peer Resource Cleanup - Prevents memory leaks
 * ✅ FIX #6: Connection Health Check - Heartbeat timeout handling
 * ✅ FIX #7: Memory leak in animation loop - Added proper cleanup
 * ✅ FIX #8: Removed duplicate variable declarations
...
/* --- Networking state (ENHANCED with heartbeat) --- */
let peer = null;
let connections = [];
let isHost = false;
...
/* --- FIX #1: Heartbeat/Keep-Alive for connection health --- */
const CONNECTION_HEARTBEAT_INTERVAL = 5000; // 5 seconds
const CONNECTION_HEARTBEAT_TIMEOUT = 3000; // 3 second timeout
let heartbeatIntervals = new Map(); // Track heartbeat for each connection
...
/* --- FIX #5: Cleanup on beforeunload with heartbeat cleanup --- */
window.addEventListener('beforeunload', () => {
    stopAutoReconnect();
    
    // Clean up all heartbeats
    heartbeatIntervals.forEach((interval, conn) => {
        clearInterval(interval);
        if (conn._pongTimeout) clearTimeout(conn._pongTimeout);
    });
    ...
    if (peer && !peer.destroyed) {
        if (isHost) {
            connections.forEach(c => { 
                try { if (c.open) c.send({ type: 'HOST_DISCONNECT' }); } catch(e){}
            });
        }
        peer.destroy();
    }
...
const VALID_NETWORK_TYPES = [
    ...
    'PING', 'PONG'  // ✅ FIX #1: Added heartbeat types
];
...
/* --- FIX #1: START CONNECTION HEARTBEAT --- */
function startConnectionHeartbeat(conn) {
    if (heartbeatIntervals.has(conn)) clearInterval(heartbeatIntervals.get(conn));
    
    let missed = 0;
    const interval = setInterval(() => {
        if (!conn || !conn.open) {
            clearInterval(interval);
            heartbeatIntervals.delete(conn);
            return;
        }
        ...
        try {
            ...
            const pongTimeout = setTimeout(() => {
                missed++;
                if (missed >= 2) {
                    console.warn(`Connection to ${conn.followerName || conn.peer} is dead (no pong).`);
                    conn.close();
                }
            }, CONNECTION_HEARTBEAT_TIMEOUT);
            
            conn._pongTimeout = pongTimeout;
        } catch (e) {
    ...
    heartbeatIntervals.set(conn, interval);
}

/* --- FIX #1: STOP CONNECTION HEARTBEAT --- */
function stopConnectionHeartbeat(conn) {
    if (heartbeatIntervals.has(conn)) {
        clearInterval(heartbeatIntervals.get(conn));
        heartbeatIntervals.delete(conn);
    }
    if (conn._pongTimeout) clearTimeout(conn._pongTimeout);
}

/* --- FIX #5: Clean all peers with resource cleanup --- */
...
/* --- NETWORK SYNC (FIX #3, #5: Improved initialization & cleanup) --- */
function startHost() {
    if (typeof Peer === 'undefined') { 
    ...
    // FIX #5: Clean up any existing connection first
    destroyAllPeers();
    stopAutoReconnect();
    ...
    peer.on('open', id => {
        ...
        peer.on('connection', conn => {
            ...
            // FIX #1: Start heartbeat monitoring for this connection
            ...
            setTimeout(() => {
                try {
                    ...
                    console.warn('Failed to send INITIAL_SYNC:', e);
                }
            }, 800);
        });
    });
...
function startJoin() {
    ...
    peer.on('open', () => {
        ...
        conn.on('open', () => {
            ...
            // FIX #4: Send username immediately on connect
...
/* --- FIX #1, #4: Enhanced setupConnection with heartbeat & improved error handling --- */
function setupConnection(conn) {
    conn.on('data', data => {
        ...
        // FIX #1: Handle heartbeat
        if (data.type === 'PING') {
            try { conn.send({ type: 'PONG', timestamp: data.timestamp }); } catch(e){}
            return;
        }
        if (data.type === 'PONG') {
            if (conn._pongTimeout) {
                clearTimeout(conn._pongTimeout);
                conn._pongTimeout = null;
            }
            return;
        }
...
/* --- FIX #3: Prevent reconnection race condition --- */
function startAutoReconnect() {
    ...
    reconnectTimer = setInterval(() => {
        ...
        // Only create new Peer if completely destroyed
        if (!peer || peer.destroyed) {
            try {
                peer = new Peer();
                peer.on('open', () => {
                    if (isReconnecting) attemptJoinOnly();
                });
                peer.on('error', (err) => {
            ...
            // Already have a valid Peer, just try to connect
            attemptJoinOnly();
        }
    }, 3000);
}

/* --- FIX #3, #4: Improved attemptJoinOnly with username persistence --- */
function attemptJoinOnly() {
    ...
    conn.on('open', () => {
        ...
        // FIX #4: Re-send username on reconnect
        conn.send({ type: 'CLIENT_INFO', username: username });
        updateStatusUI();
        showToast(`Reconnected to ${currentHostId}`, "success");
    });

    conn.on('error', (err) => {
        clearTimeout(failTimeout);
        isAttemptingJoin = false;
        console.warn('Reconnection attempt failed:', err.type);
    });
}
...
/* --- FIX #2: Improved Broadcast with pre-validation & better error handling --- */
function broadcast(data) {
    if (!isHost || !connections.length) return;
    
    // Pre-filter: Remove all closed/invalid connections BEFORE sending
    ...
    // Now send to all valid connections
    ...
    validConnections.forEach(conn => {
        try {
            if (conn.open) {
                ...
                failedConnections.push(conn);
            }
        } catch (e) {
            console.warn(`Failed to send to ${conn.followerName || 'unknown'}: ${e.message}`);
            failedConnections.push(conn);
        }
    });
    ...
    if (failedConnections.length > 0) {
        connections = connections.filter(c => !failedConnections.includes(c));
        updateStatusUI();
    }
}
