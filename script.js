...
function saveToDisk() {
    ...
    safeSetLocal(PROJECTS_KEY, JSON.stringify(allProjects));
    // Only broadcast the active project's cues (not all projects) to reduce payload
    if (isHost && activeProjectId !== null) {
        const proj = allProjects.find(p => p.id == activeProjectId);
        if (proj) broadcastCues(proj.cues);
    }
}
...
function startHost() {
    ...
    // Only sync active project, not all
    if (activeProjectId !== null) {
        const proj = allProjects.find(p => p.id == activeProjectId);
...
function startJoin() {
    ...
    addListener('nav', 'value', snap => {
        ...
        if (nav.page === 'project' && nav.projectId) {
            // For followers, projectId might not exist locally — create a stub
            let proj = allProjects.find(p => p.id == nav.projectId);
            if (!proj) {
                proj = { id: nav.projectId, name: 'Host Project', cues: [] };
                allProjects.push(proj);
            }
            activeProjectId = nav.projectId;
            document.getElementById('active-project-name').innerText = sanitizeText(proj.name || 'Host Project');
            document.getElementById('page-projects').classList.remove('active');
            document.getElementById('page-cues').classList.add('active');
...
// Only used by host: broadcast just the active project's cues (smaller payload)
