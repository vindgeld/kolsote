...
function renderProjects() {
    ...
    if (!allProjects.length) {
        list.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:60px 20px;color:var(--muted);">
            ...
            <button class="panel-item" onclick="createNewProject()" style="display:inline-block;">＋ NEW PROJECT</button>
        </div>`;
        return;
    }
...
function setupFirebase() {
    ...
    currentRoomId = localStorage.getItem(ROOM_KEY) || 'default';
...
function startJoin() {
    ...
    addListener('allProjects', 'value', snap => {
        ...
        // Merge host projects into local allProjects by id
        // (preserve any local-only projects the follower might have)
        data.forEach(hostProj => {
            const localIdx = allProjects.findIndex(p => p.id == hostProj.id);
            if (localIdx !== -1) {
                allProjects[localIdx] = hostProj; // update in place
            } else {
                allProjects.push(hostProj);       // new project from host
            }
        });
        // Remove projects that host has deleted (only host-originated ones)
        const hostIds = data.map(p => String(p.id));
        allProjects = allProjects.filter(p => {
            // Keep if host still has it, or if it was never from host
            return hostIds.includes(String(p.id)) ||
                   !data.some(hp => String(hp.id) === String(p.id));
        });
        renderProjects();

        // If we are inside a project, refresh its cues from the updated list
        if (activeProjectId !== null) {
            const proj = allProjects.find(p => p.id == activeProjectId);
            if (proj) {
                cueData = proj.cues ? proj.cues.map(c => Object.assign({}, c)) : [];
    ...
    addListener('nav', 'value', snap => {
        ...
        if (nav.page === 'project' && nav.projectId) {
            ...
            if (proj) {
                ...
                // Fallback stub if allProjects hasn't arrived yet
                cueData = [];
