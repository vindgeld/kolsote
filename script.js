...
function startJoin() {
    ...
    addListener('allProjects', 'value', snap => {
        ...
        // Track which ids we knew came from the host (any id present last sync)
        // Simple rule: replace allProjects entirely with host list, keeping only
        // local-only projects (ones the host has never broadcast) alongside.
        // A project is "host-originated" if we received it from host at any point.
        // We track that via the knownHostIds Set that persists across calls.
